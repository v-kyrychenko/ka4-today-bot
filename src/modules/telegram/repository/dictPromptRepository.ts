...
export const dictPromptRepository = {
    async getPromptByKey(key: string): Promise<PromptDict> {
        ...
        const systemPrompt = prompt.sys_prompt_id != null ? await findPromptById(prompt.sys_prompt_id) : null;
...
async function findPromptByKey(key: string): Promise<DictPromptRow | null> {
    const [row] = await getPostgresDb().select().from(dictPrompt).where(eq(dictPrompt.key, key)).limit(1);

    return (row) ?? null;
}

async function findPromptById(id: number): Promise<DictPromptRow | null> {
    const [row] = await getPostgresDb().select().from(dictPrompt).where(eq(dictPrompt.id, id)).limit(1);
