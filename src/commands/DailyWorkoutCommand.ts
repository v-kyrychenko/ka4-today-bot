...
import {Exercise, ExerciseWithSignedImages, type ProcessorContext} from '../models/app.js';
...
export class DailyWorkoutCommand extends BaseCommand {
    ...
    async execute(context: ProcessorContext): Promise<void> {
        ...
        if (!scheduled) {
            const reply = await openAiService.fetchOpenAiReply({
                context,
        ...
        if (plan == null) {
            const message = await openAiService.fetchOpenAiReply({
                context,
                promptRef: PROMPT_REF_NOT_NO_PLAN,
            });
            await telegramService.sendMessage(context, message);
            return;
        }
