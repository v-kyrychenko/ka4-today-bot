...
const routeMocks = {
    ...
    setup(buildContext) {
        ...
        mockModule(buildContext, /dailyNutritionPlanner\.js$/, [
            'export async function buildDailyNutritionContext(context) {',
            '    const clientId = context.user.clientId ?? null;',
            '    if (clientId == null) return null;',
            '    globalThis.__dailyMealsRouteMocks.calls.push(["findByClientId", clientId]);',
            '    const client = globalThis.__dailyMealsRouteMocks.client;',
            '    if (client == null) return null;',
            '    if (client.height == null || client.height <= 0) {',
            '        const text = context.user.lang === "en"',
            '            ? "To build your menu, I need your height. Add it to your profile, and I will generate the nutrition plan right away."',
            '            : "Щоб скласти меню, мені потрібен твій зріст. Додай зріст у профіль, і я одразу згенерую план харчування."; ',
            '        globalThis.__dailyMealsRouteMocks.calls.push(["send", context.chatId, text]);',
            '        return null;',
            '    }',
            '    globalThis.__dailyMealsRouteMocks.calls.push(["findLatestForClientByType", clientId, "WEIGHT"]);',
            '    const weight = globalThis.__dailyMealsRouteMocks.weight ?? null;',
            '    if (weight == null) {',
            '        globalThis.__dailyMealsRouteMocks.calls.push(["send", context.chatId, "Щоб скласти меню, мені потрібна актуальна вага. Надішли заміри, і я одразу згенерую план харчування."]);',
            '        return null;',
            '    }',
            '    globalThis.__dailyMealsRouteMocks.calls.push(["getUserScheduledForDay", context.chatId]);',
            '    const dayType = globalThis.__dailyMealsRouteMocks.scheduled == null ? "rest_day" : "training_day";',
            '    return {',
            '        clientId,',
            '        gender: client.gender,',
            '        birthday: client.birthday,',
            '        goal: client.goals?.trim() || "maintenance",',
            '        weight,',
            '        height: client.height,',
            '        activityLevel: dayType === "training_day" ? "active" : "low_active",',
            '        dayType,',
            '    };',
            '}',
